...
To verify a downloaded release asset, download it together with `SHA256SUMS` from the same GitHub Release, then run `shasum -a 256 -c SHA256SUMS`. GitHub CLI users can additionally verify the build provenance generated by the release workflow:

```bash
gh attestation verify ./myminivault_vX.Y.Z_linux-amd64.tar.gz \
  --repo olelbis/myminivault \
  --signer-workflow .github/workflows/release-packages.yml
```

The release workflow checks out and builds the release tag itself, verifies that the checked-out commit matches that tag, and disables persisted checkout credentials. Its upload and attestation permissions are granted only to the jobs that need them.
