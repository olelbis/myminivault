...
```bash
go build -trimpath -ldflags="-s -w -X main.vaultVersion=0.12.12" -o bin/vault ./cmd/vault
```
...
9. Create the GitHub release with a title matching only the tag, such as `v0.12.12`.
