...
Run the container parser fuzz target after format or metadata parser changes:

```bash
go test ./internal/container -run=FuzzParse -fuzz=FuzzParse -fuzztime=10s
```
...
9. Create the GitHub release with a title matching only the tag, such as `v0.13.1`.
