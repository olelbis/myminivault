...
The actively supported operating-system targets are macOS and Linux. Windows is not fully supported yet: there are no Windows release packages, Windows CI jobs, or reviewed Windows-specific guarantees for file locking, runtime permissions, no-follow opens, or OS key storage.
...
- derive main-vault encryption keys from a master password using Argon2id for new password-based saves, while keeping scrypt readable for deprecated compatibility formats
- derive recovery and shared-token vault encryption keys with HKDF-SHA256 when the input is generated high-entropy key material
...
File permissions are an important local mitigation, not a complete security boundary. Normal startup tightens existing runtime files to `0600` when possible and fails early if a critical sensitive path cannot be secured. Sensitive runtime paths are rejected when they are symbolic links, checked runtime file opens use no-follow semantics on Unix-like systems, and `vault doctor` reports symlinked sensitive files as failures. `vault doctor` and `vault inspect-runtime` are intentionally non-mutating and report the current state without auto-fixing it. They also report recovery freshness and non-decrypting recovery container compatibility so recovery drift is visible before the recovery path is needed.

Sensitive runtime paths are checked with portable symlink rejection before important reads and writes, and checked opens use OS-level no-follow flags where supported. Main-vault transaction markers plus main-vault, restore, recovery, and shared-token-vault temp files are created with exclusive-create semantics, so a pre-existing temp or marker path causes the command to fail instead of truncating or reusing that path. A runtime directory controlled by another process under the same account, or an intentionally unsafe `MYMINIVAULT_HOME`, may still expose file-replacement and rollback races outside those checked opens. Use a private local runtime directory and do not treat `0700`/`0600` modes as protection from the same OS user.

Newly saved encrypted runtime files include a small cleartext `MYMV` container header with a container format version, file kind, and non-sensitive crypto metadata. Current `MYMV v2` saves record details such as `AES-256-GCM`, the selected KDF, salt size, nonce size, and payload layout. Password-based main-vault saves use the configured `kdf` policy, defaulting to Argon2id with validated parameters from `vault-config.json`. Recovery snapshots and shared-token vaults use HKDF-SHA256 for new saves because their inputs are generated high-entropy key material. The `MYMV v2` header, metadata, and salt are authenticated as AES-GCM additional authenticated data, so tampering with that cleartext context makes decryption fail. Load paths validate supported algorithm, KDF, payload, ciphertext layout, nonce size, and bounded KDF parameters before deriving keys from MYMV v2 metadata. scrypt remains available for explicit compatibility testing and legacy fallback. The Argon2id metadata policy accepts memory from 19 MiB through 256 MiB: the lower bound prevents attacker-edited headers from weakening current password-based files, while the upper bound prevents hostile headers from forcing excessive memory allocation before authentication can complete. scrypt-based `MYMV v2`, Argon2id recovery/shared-token vaults from older experimental releases, legacy, and MYMV v1 files remain readable for compatibility but are deprecated and rewritten with the current write profile after normal authenticated save operations. This supports safer inspection and explicit deprecation windows without requiring a separate mutating migration path. The header reveals that a file is a myminivault encrypted container and whether it is a main, recovery, or shared-token vault file; it does not reveal key names, values, recovery metadata, token contents, or encrypted vault metadata.
...
GitHub Releases publish source tags, binary archives, and installable packages. Per-target SPDX JSON SBOMs document Go module dependencies for each release target. Per-target checksum files and the aggregate `SHA256SUMS` manifest help detect accidental corruption or mismatched downloads. Release workflow artifacts also receive GitHub artifact attestations, which provide signed build provenance for assets produced by GitHub Actions. The release workflow checks out the named release tag, confirms that `HEAD` resolves to that tag's commit before building, and does not persist checkout credentials. Its release-upload and attestation permissions are scoped to the jobs that require them. GitHub Actions are pinned to immutable commit SHAs in workflows to reduce tag-retargeting risk. `staticcheck`, CodeQL, and `govulncheck` run in GitHub Actions to catch common static-analysis findings and known Go vulnerabilities. `gosec` is used as a local triage tool with documented suppress/accept rules before it becomes a gate. These automated checks are not an external security audit.
...
| --- | --- | --- | --- |
| `vault.db` | High | Offline password guessing, copied encrypted secrets | AES-GCM encryption, Argon2id for new password-based saves, restrictive writes |
...
| `vault.db.recovery` | High | Recovery-encrypted snapshot exposure | High-entropy recovery key, HKDF-SHA256 for new saves, restrictive writes |
...
| `shared-token-vault.json` and `.bak` | High | Token-access vault exposure | Encrypted current and previous shared vault, HKDF-SHA256 from token master key |
...
- Argon2id key derivation for new password-based main-vault saves, with scrypt compatibility reads for deprecated files
...
- `vault inspect-runtime` summarizes the recovery/main-vault relationship and helps identify active versus legacy runtime files
...
- v2 HMAC token signatures verified with the local token master key before shared-vault loading
- legacy HMAC token signature compatibility after shared-vault loading
...
| --- | --- |
| Copied encrypted vault file | Mitigated by Argon2id and AES-GCM for new password-based main-vault saves, but weak passwords remain risky |
...
- continue reducing direct process-argument exposure where practical
- keep expanding crash-consistency tests around directory sync and interrupted writes
- keep extending deprecated-format coverage around bounded KDF metadata loading and legacy compatibility
